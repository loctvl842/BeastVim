...
local function record(target, name, total, self_t)
    ...
    if total < s.min then s.min = total end
    if total > s.max then s.max = total end
end
...
local function wrap_function(name, fn, target_stats)
    return function(...)
        if not state.recording then return fn(...) end
        ...
        if parent then parent.child_total = parent.child_total + total end
...
local function hooked_require(name)
    ...
    if type(mod) == "table" then wrap_module(name, mod) end
    return mod
end

local function install_require_hook()
    if state.rawrequire then return end
...
function M.start(opts)
    ...
    for name, mod in pairs(package.loaded) do
        if should_profile(name) then wrap_module(name, mod) end
    end
end
