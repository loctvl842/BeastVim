...
- Last updated: 2026-09-04 (explorer's clipboard copy/cut marker switched from an inline `(copy)`/`(cut)` name suffix to a right-aligned badge glyph — `config.icon.clip` — sharing the same virt_text row as the git/diagnostic badges; plus prior: active-file indicator switched from a `BeastExplorerActiveFile` background tint to a configurable `┃` left-gutter marker glyph — `config.icon.active_file`)
